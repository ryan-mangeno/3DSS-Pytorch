...
class DatasetViewer:
    ...
    def transform_x(self, x):
        ...
        # from [C, H, W] to [H, W, C] - only for RGB images, our dataviewer expects this format
        if self.check_if_rgb(x):
            x = np.moveaxis(x, source=0, destination=-1)

        # normalizing for preprocessing, re_normalize normalizes from [0,255] by default (transfromations.py)
        x = re_normalize(x)
